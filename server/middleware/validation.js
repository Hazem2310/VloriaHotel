...
export const validateRoomBooking = (req, res, next) => {
  const { room_id, start_date, end_date, adults = 1, children = 0 } = req.body;
  ...
  req.validatedBooking = {
    ...
    meal_package_id: req.body.meal_package_id
      ? parseInt(req.body.meal_package_id)
      : null,
...
export const validateRestaurantBooking = (req, res, next) => {
  ...
  if (!restaurant_id || !reservation_datetime || !party_size) {
    return res.status(400).json({
      success: false,
      message:
        "Please provide restaurant_id, reservation_datetime, and party_size",
    });
  }
...
export const validateEmployeeCreation = (req, res, next) => {
  ...
  const validRoles = ["customer", "employee", "manager", "admin", "owner"];
...
export const validateRoomCreation = (req, res, next) => {
  ...
  req.validatedRoom = {
    ...
    extra_bed_allowed:
      req.body.extra_bed_allowed === true || req.body.extra_bed_allowed === 1,
    extra_bed_price: req.body.extra_bed_price
      ? parseFloat(req.body.extra_bed_price)
      : 0,
    notes: req.body.notes || null,
